...
def _load_events(file_path: Path) -> list:
    events = []
...
def _load_rates(file_path: Path) -> dict:
    rates = {}
    with open(file_path) as f:
        for line in f:
            parts = line.strip().split("|")
...
EVENTS = _load_events(SCRIPT_DIR / "data/events.txt")
TELESCOPE_RATES = _load_rates(SCRIPT_DIR / "data/telescope_rates.txt")
PRIORITY_MULTIPLIERS = _load_rates(SCRIPT_DIR / "data/priority_multipliers.txt")
